...
\screenshot{transition-adminer-oltp-vs-etl.png}{Vue côte à côte sur Adminer :
le schéma \code{ops} normalisé (cible \emph{Orion OLTP}) à gauche, et la base
\code{OrionETL} (\code{staging}, \code{dim}, \code{fact}, cible \emph{OrionETL
SQL Server}) à droite.}
...
\begin{itemize}
  ...
  \item \code{adminer}       : UI web unique (Adminer 5) pour Postgres OLTP,
        Postgres DWH \emph{et} SQL Server. Trois cibles pré-configurées via
        un index.php personnalisé (sélecteur \og Cible Orion \fg{}) :
        \emph{Orion OLTP}, \emph{Orion DWH}, \emph{OrionETL}.
\end{itemize}

\begin{figure}[h]
\centering
\diag{06-docker-stack}
\caption{Stack Docker : Postgres OLTP, SQL Server hébergeant la logique ETL, Postgres DWH, orchestrateur Python qui relie les trois, plus Adminer (UI multi-SGBD) et data-gen.}
\end{figure}

\screenshot{docker-compose-ps.png}{Sortie de \texttt{docker compose ps} : les cinq services à l'état \emph{Up (healthy)}, plus \texttt{data-gen} qui ne tourne que sous \texttt{--profile seed}.}

\screenshot{adminer-login.png}{Adminer --- écran de connexion : sélection de la cible (\emph{Orion OLTP / Orion DWH / OrionETL}) puis identifiants. Un seul outil pour les trois SGBD.}
...
services:
  ...
  adminer:                         # UI web multi-SGBD (pgsql + mssql)
    image: adminer:5
    environment:
      ADMINER_DESIGN: ${ADMINER_DESIGN}
      ADMINER_DEFAULT_DRIVER: pgsql
      ADMINER_DEFAULT_SERVER: postgres-oltp
    ports: ["${ADMINER_PORT}:8080"]
    volumes:
      - ./adminer/index.php:/var/www/html/index.php:ro
\end{lstlisting}

\paragraph{Adminer en lieu et place de pgAdmin.}
Adminer présente trois avantages dans ce TP :
\begin{itemize}
  \item un \textbf{seul outil} pour les trois SGBD (Postgres OLTP, Postgres
        DWH, SQL Server \code{OrionETL}), au lieu d'alterner pgAdmin + SSMS ;
  \item \textbf{sans état} (aucun volume), démarrage en quelques secondes ;
  \item un \texttt{adminer/index.php} personnalisé pré-remplit la liste des
        cibles (\emph{Orion OLTP}, \emph{Orion DWH}, \emph{OrionETL}) au login.
\end{itemize}
...
\code{mssql\_data} : la persistance survit à \code{docker compose down}
mais peut être effacée par \code{down -v}. \emph{Adminer est sans état},
...
# 1) Bases + SQL Server + orchestrator + Adminer
...
|       |-- 05_sp_dim_simple.sql   (sp_load_dim_date/canal/geo/...)
|       |-- 06_sp_dim_scd2.sql     (sp_load_dim_client/employe SCD2)
|       `-- 07_sp_fact.sql         (sp_load_fait_ventes + sp_run_pipeline)
|-- orchestrator/                  ** transport Postgres <-> SQL Server **
|   |-- Dockerfile                 (python:3.12 + msodbcsql18)
|   |-- requirements.txt
|   `-- orchestrate.py             (extract / EXEC / push, APScheduler)
|-- adminer/                       ** UI web multi-SGBD **
|   `-- index.php                  (3 cibles pre-configurees)
`-- analytics/
...
\screenshot{adminer-dwh-fact.png}{Adminer --- aperçu du DWH après chargement : ligne d'\texttt{dw.fait\_ventes} avec ses surrogate keys et mesures (cible \emph{Orion DWH}).}
