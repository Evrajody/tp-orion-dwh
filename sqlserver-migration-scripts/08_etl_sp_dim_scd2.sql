...
BEGIN
    ...
    BEGIN TRY
            ...
            SELECT
                ...
                CONVERT(CHAR(64), HASHBYTES('SHA2_256',
                    CONCAT_WS(N'|',
                        ...
                        ISNULL(type_contrat_courant, N''),
                        ISNULL(CONVERT(NVARCHAR, date_debut_contrat), N''),
                        ISNULL(CONVERT(NVARCHAR, date_fin_contrat),   N''),
                        ISNULL(statut_contrat, N''),
                        ISNULL(CONVERT(NVARCHAR, nb_contrats_signes), N''))
                ), 2) AS hash_ligne
        ...
        SELECT s.employe_id, s.nom_complet, s.sexe, s.tranche_age,
               ...
               s.type_contrat_courant, s.date_debut_contrat, s.date_fin_contrat,
               s.statut_contrat, s.nb_contrats_signes,
               @ajd, NULL, 1, s.hash_ligne
          FROM src s
